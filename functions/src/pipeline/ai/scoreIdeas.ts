...
import { RawIdea, ScoredIdea, DecisionTier } from '../../types/pipeline.js';
 ...
 */
function calculateComposite(idea: Pick<ScoredIdea, 'businessPotential' | 'developmentComplexity' | 'timeToMarket' | 'competitionLevel' | 'riskLevel'>): number {
  const weights = {
  ...
  const score =
    (idea.businessPotential || 3) * weights.businessPotential +
 ...
 */
function getTier(score: number): DecisionTier {
