...
import { PolymarketData } from '../../types/pipeline.js';
...
export async function fetchPolymarketSignals(): Promise<PolymarketData> {
  ...
  const markets = await marketsResponse.json() as PolymarketMarket[];
  ...
  const filteredMarkets = markets.filter((m: PolymarketMarket) =>
    relevantCategories.some(cat => m.category?.toLowerCase().includes(cat.toLowerCase()))
  );
  ...
  const structuredMarkets = filteredMarkets.slice(0, 50).map((m: PolymarketMarket) => ({
