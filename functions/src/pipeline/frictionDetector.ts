...
import { getStoredReviews, clearStoredReviews } from './sources/appstore.js';
...
interface ExtractedFriction {
  category: FrictionCategory;
  description: string;
  severity: number; // 1-5
  frequency: number; // count
  userQuotes: string[];
  aiAddressability: 'high' | 'medium' | 'low';
  keywords: string[];
}
 ...
 */
function determinePriority(score: number): FrictionPriority {
 ...
 */
function frequencyToScore(count: number): number {
 ...
 */
function addressabilityToSolutionTypes(
  addressability: 'high' | 'medium' | 'low',
  category: FrictionCategory
): AISolutionType[] {
  const categoryMappings: Record<FrictionCategory, AISolutionType[]> = {
 ...
 */
async function extractFrictionFromReviews(
  batch: ReviewBatch,
  geminiApiKey: string,
  runId: string
): Promise<ExtractedFriction[]> {
...
Return at least 3 friction points, max 10. Only include distinct issues.`;
  ...
  try {
    const parsed = JSON.parse(data.candidates[0].content.parts[0].text) as ExtractedFriction[];
 ...
 */
function aggregateFrictionPoints(
  frictionsByApp: Map<string, ExtractedFriction[]>,
  runId: string
): FrictionPoint[] {
  const aggregated: FrictionPoint[] = [];
  let frictionId = 1;

  // Group by category + similar description
  const categoryGroups = new Map<FrictionCategory, ExtractedFriction[]>();
  ...
  for (const [category, frictions] of categoryGroups) {
    ...
    const clusters: ExtractedFriction[][] = [];
    ...
    for (const cluster of clusters) {
      ...
      const addressability = (Object.entries(addressabilityCounts)
        .sort((a, b) => b[1] - a[1])[0][0]) as 'high' | 'medium' | 'low';
      ...
      const appInfo = [...frictionsByApp.entries()].find(([, frictions]) =>
        frictions.includes(cluster[0])
      );
...
function generateSuggestedApproach(
  ...
  const approaches: Record<FrictionCategory, string> = {
 ...
 */
export async function detectFriction(runId: string): Promise<FrictionPoint[]> {
  ...
  const frictionsByApp = new Map<string, ExtractedFriction[]>();
  let totalApiCalls = 0;
  ...
  for (const { appId, appName, reviews } of storedReviews) {
    ...
    const appFrictions: ExtractedFriction[] = [];
