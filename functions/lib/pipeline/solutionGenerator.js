...
exports.generateAISolutions = generateAISolutions;
// ============================================
// CONFIGURATION
// ============================================
 ...
 */
function clusterFrictionPoints(frictionPoints, runId) {
    ...
    for (const [category, points] of categoryGroups) {
        ...
        for (const point of points) {
            ...
            const related = points.filter(p => {
                ...
                const solutionOverlap = point.aiAnalysis.solutionTypes.some(s => p.aiAnalysis.solutionTypes.includes(s));
 ...
 */
function generateThemeName(category, description) {
    const categoryNames = {
 ...
 */
async function generateSolutionsFromClusters(clusters, niches, ideasPerNiche, geminiApiKey, runId) {
...
Be creative with naming but realistic with technical claims.`;
    ...
    try {
        const solutions = JSON.parse(data.candidates[0].content.parts[0].text);
...
function transformToAINativeIdeas(solutions, clusters, niches, runId) {
    ...
    return solutions.map((solution, index) => {
        ...
        const compositeScore = scores.businessPotential * 0.25 +
        ...
        if (cluster?.isIndustryWide)
            adjustedScore += 0.3;
        if (solution.heroFeature.automationLevel === 3)
            adjustedScore += 0.2;
        if (solution.aiApproach.effortReduction >= 90)
            adjustedScore += 0.2;
        // Determine tier
        let tier;
        if (adjustedScore >= 4.0)
            tier = 'hot';
        ...
        const aiApproach = {
            ...
            automationLevel: solution.heroFeature.automationLevel,
 ...
 */
async function generateAISolutions(frictionPoints, niches, ideasPerNiche, runId) {
    ...
    const highPriorityFriction = frictionPoints.filter(fp => fp.priority === 'P0' || fp.priority === 'P1');
    console.log(`[SolutionGenerator ${runId}] High-priority friction points: ${highPriorityFriction.length}`);
    // Cluster friction points
    const clusters = clusterFrictionPoints(highPriorityFriction.length > 0 ? highPriorityFriction : frictionPoints.slice(0, 10), runId);
...
//# sourceMappingURL=solutionGenerator.js.map